...
void MADtreeV::MADNodeHandle::build(const ContainerTypePtr vec,
  ...
  if (true || level >= max_parallel_level) {
    add_left().build(vec, begin, middle, b_max, b_min, level + 1, max_parallel_level, idx, plane_predecessor);

    add_right().build(vec, middle, end, b_max, b_min, level + 1, max_parallel_level, idx, plane_predecessor);
  }
...
void MADtreeV::MADNodeHandle::apply_transform(const Eigen::Matrix3d& r, const Eigen::Vector3d& t) {
  ...
  if (has_left())
    left().apply_transform(r, t);
  if (has_right())
    right().apply_transform(r, t);
}
